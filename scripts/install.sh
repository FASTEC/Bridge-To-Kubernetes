...
download_bridge_stable_version() {
    ...
    if [[ $OSTYPE == "linux"* ]]; then
        ...
        curl --create-dirs -o $HOME/tmp/bridgetokubernetes/lpk-osx.zip -LO "https://github.com/go1com/Bridge-To-Kubernetes/releases/download/${VERSION}/osx-x64.zip"
        elif [[ $OSTYPE == "win"* ]] || [[ $OSTYPE == "msys"* ]]; then
        curl --create-dirs -o $HOME/tmp/bridgetokubernetes/lpk-win.zip -LO "https://github.com/go1com/Bridge-To-Kubernetes/releases/download/${VERSION}/win-x64.zip"
    else
...
file_issue_prompt() {
    ...
    echo "https://github.com/go1com/Bridge-To-Kubernetes/issues/new/choose"
    exit 1
}
